...
<table>
  ...
  <?php 
  $res=$con->query("SELECT * from clt");
  if($res->num_rows>0)
  {
	  while($row=$res->fetch_assoc())
	  {
		  echo "<tr><td>".$row['clt_id']."</td><td>".$row['clt_name']."</td><td>".$row['clt_mobno']."</td><td>".$row['clt_email']."</td><td>".$row['clt_address']."</td><td>".$row['clt_branch']."</td><td>".$row['clt_work']."</td><td>".$row['emp_id']."</td></tr>";
	  }
	  echo "</table>";
  }
